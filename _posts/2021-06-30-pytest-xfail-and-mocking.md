---
title: pytest.mark.xfail과 mock.patch
tags: python TIL
---

<br/>
view, service의 로직을 테스트하는 방법에 대해 오늘 배운 것. 나름 테스트 코드 심화 😤 <br/>
<!--more-->

테스트 코드를 쓰다 보면 정상적으로 실행되는 케이스 외에 아래 케이스도 확인해야 하게 된다.

- 에러가 나야 할 곳에서 정상적으로 에러가 나는지 확인
- 조건을 fixture로 만들 수 없거나, 서비스 로직에 http 요청이 포함되어 유닛 테스트에서 케이스를 만들 수 없는 경우

오늘 이 두 가지 케이스를 모두 접하게 되어 TIL로 간단히 정리하고자 한다.

# 테스트란?
[pytest 공식 문서](https://docs.pytest.org/en/6.2.x/fixture.html#fixture)에 테스트에 대해 잘 정리되어 있어서 가져왔다.
테스트는 특정 행동에 대한 결과를 보도록 되어있고, 결과가 내가 기대한 것과 동일한지를 확인하는 것이다.

테스트 코드를 쓰는 것이 힘든 이유는, 행동(이하 Behavior)은 측정될 수 있는 것이 아니기 때문이다. Behavior은 특정 상황에서 시스템이 반응하는 것이지만,
정확히 어떻게 왜(how? why?) 되는지보다 무엇이) 되었는지(what is done)가 더 중요하다.  테스트는 아래 네 단계로 나눠볼 수 있다.

## 테스트의 단계
테스트는 오직 context만 제공하는 arrange 단계 + act과 assert 단계들이며, Behavior는 act와 assert 사이에 존재한다.
...
테스트 이후 스스로를 pick up해 다른 테스트에 영향이 가지 않게 하는 단계다.

# pytest
pytest 공식 문서에 보면 큰 꼭지가 Fuctions, Marks, Fixtures, Hooks, Objects가 있다. 이 중에서 자주 쓰는 Marks와 Fixtures를 정리하고 시작하자.
...
- argument로 선언함으로써 테스트 함수나 다른 fixture에 의해 요청될 수 있는 것이다.
...
### Marks
- fixtures나 plugins를 통해 접근할 수 있는 테스트 함수에 메타 데이터를 적용하는 데에 사용될 수 있다.
...
skip이나 xfail은 성공할 수 없는 테스트를 다루는 방식으로, 특정 플랫폼에서 실행될 수 없거나 실패를 기대하는 테스트 함수에 표시해를 해서 테스트 요약에 나타낸다.

- skip은 특정 조건에 맞아야 성공하고, 그렇지 않으면 테스트도 스킵하는 것이다. 일반적인 예는 윈도우가 아닌 플랫폼에서 윈도우 only test를 skip하는 것이다.
- xfail은 특정 이유로 테스트가 실패하는 것을 기대하는 것을 의미한다. 기능을 위한 테스트가 아직 실행되지 않았거나, 버그가 수정되지 않은 것 등이다.  실패를 기대했는데도 만약 패스한다면, test summary에 xpass로 리포팅되게 된다.
...
정상적으로 에러가 발생하는 케이스라면 테스트를 실행했을 때 해당 테스트에 traceback이 찍혀있어야 하는데, 아무 로그도 없는 것을 보고 바로 수정하게 됐다.
그래서 다음 방식이 더 좋을 것 같다.

#### 개선점

- 에러 검증을 위해 테스트 함수 내옹을`pass` 보다는 `assert False`를 통해 테스트 통과 여부를 명시적으로 알 수 있도록 해야 함
- pytest.mark.xfail은 에러가 나지 않아도 통과된다는 점을 인지해야 함
...
def test_function():
    ...
    # 만약 False가 되면 에러가 나고, 정상 동작했다는 것을 의미한다.
```
...
함수 혹은 클래스 데코레이터나 컨텍스트 매니저로 사용될 수 있다. 첫 번째 인자가 target인데, 함수 바디나 with statement 내에서 target은 new object와 함께 patch된다.
쉽게 말해 테스트 시 검증이 불가한 부분만 변경해주는 것인데, patch는 프로그래밍에서 부분적 변경을 의미한다. mocking을 통해 부분적으로 변경해서 테스트를 진행하는 것이라고 이해하면 좋을 것 같다.

### new
아래 함수는 http 요청이 필요한 async_request_service 대신, 해당 로직을 담고 있는 함수인 request_function을 new 인자에 넣어 테스트를 하는 예시다.
mock 부분만 보여주기 위한 예시로, 해당 fixture를 Act 함수의 인자로 넣어 함께 실행하면 된다. new에 대신 실행을 원하는 함수명을 입력한다.
...
잘 쓰여진 테스트 코드를 많이 읽으면서 배우고 싶다는 생각을 했다.
